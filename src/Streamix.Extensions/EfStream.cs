...
{
    static async IAsyncEnumerable<T> executeQuery<T>(Func<DbContext, IQueryable<T>> query, Func<DbContext> dbContextFactory,
        ...
        string? name = null) where T : class
    {
        ...
        return Stream.From(executeQuery(query, dbContextFactory), name);
    }
        ...
        string? name = null) where T : class
    {
        ...
        return Stream.From(
            executeQuery(query, dbContextFactory),
            clock,
            name);
    }
}
