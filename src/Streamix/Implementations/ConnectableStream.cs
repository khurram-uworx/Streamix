...
class ConnectableStream<T> : IConnectableStream<T>
{
    ...
    {
        // Use a small buffer to avoid race conditions in the happy path where producer and consumer start nearly simultaneously.
        var channel = Channel.CreateBounded<T>(16);
        using var cts = CancellationTokenSource.CreateLinkedTokenSource(cancellationToken);
        ...
        var producerTask = Task.Run(async () =>
        {
            try
            {
                await foreach (var item in this.WithCancellation(cts.Token))
                {
                    if (!channel.Writer.TryWrite(item))
                    {
                        throw new BackpressureException("Downstream cannot keep pace (OnBackpressureError strategy).");
                    }
                }
                channel.Writer.Complete();
            }
