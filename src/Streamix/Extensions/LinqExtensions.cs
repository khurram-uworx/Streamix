...
public static class LinqExtensions
{
    ...
    /// LINQ-style extension for <see cref="IStream{T}.ConcatMap{TResult}(Func{T, IStream{TResult}})"/>.
    /// </summary>
    /// <typeparam name="T">The type of items in the stream.</typeparam>
    /// <typeparam name="TResult">The type of the elements in the resulting stream.</typeparam>
    ...
    /// <summary>
    /// Projects each element of a stream to an <see cref="IStream{TResult}"/> and flattens the resulting streams into one stream.
    /// LINQ-style extension for <see cref="IStream{T}.FlatMapOrdered{TResult}(Func{T, IStream{TResult}}, int)"/> with concurrency support.
    /// </summary>
    /// <typeparam name="T">The type of items in the stream.</typeparam>
    /// <typeparam name="TResult">The type of the elements in the resulting stream.</typeparam>
    /// <param name="source">The stream to flatten.</param>
    ...
    public static IStream<TResult> SelectMany<T, TResult>(this IStream<T> source, Func<T, IStream<TResult>> selector, int maxConcurrency)
        => source.FlatMapOrdered(selector, maxConcurrency);
    ...
    {
        return source.Map(item => selector(item).AsTask(), maxConcurrency: 1).ConcatMap(stream => stream);
    }
    ...
    {
        return source.Map(item => selector(item).AsTask(), maxConcurrency).FlatMap(stream => stream);
    }
