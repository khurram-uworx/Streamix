...
{
    enum ProcessingTimeSignalKind { Item, Tick, SourceCompleted, SourceFault, }

    readonly record struct ProcessingTimeSignal<T>(ProcessingTimeSignalKind Kind,
        T? Item = default, Exception? Error = null)
    {
    ...
    public static IFlux<IList<T>> BufferByTime<T>(this IFlux<T> source, TimeSpan interval, int? maxCount = null)
    {
        ...
        return Flux.Create<IList<T>>(async (emitter, ct) =>
        {
            ...
            await processingTimeCoordinatorRunAsync(source, interval, ct, async signal =>
            {
                switch (signal.Kind)
                {
                    case ProcessingTimeSignalKind.Item:
                    ...
                    case ProcessingTimeSignalKind.Tick:
                        if (buffer.Count > 0)
                        {
                            await emitter.EmitAsync(buffer).ConfigureAwait(false);
                            buffer = new List<T>(maxCount ?? 16);
                    ...
                    case ProcessingTimeSignalKind.SourceCompleted:
                        if (buffer.Count > 0)
                        {
                            await emitter.EmitAsync(buffer).ConfigureAwait(false);
                        }
                    ...
                    case ProcessingTimeSignalKind.SourceFault:
                        emitter.Fail(signal.Error!);
                        return false;

                    default:
                        throw new InvalidOperationException($"Unknown signal kind: {signal.Kind}.");
                }
