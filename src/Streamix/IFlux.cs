...
public interface IFlux<T> : IAsyncEnumerable<T>
{
    /// <summary>
    /// </summary>
    ...
    /// <returns>A new <see cref="IFlux{T}"/> with the specified name.</returns>
    IFlux<T> Named(string name);

    /// <summary>
    /// Merges this stream with other streams into a single stream.
    /// </summary>
    /// <param name="others">The other streams to merge with.</param>
    /// <returns>A merged <see cref="IFlux{T}"/>.</returns>
    IFlux<T> MergeWith(params IFlux<T>[] others);
    ...
    /// <returns>An <see cref="IFlux{TResult}"/> that contains zipped elements of the two streams.</returns>
    IFlux<TResult> ZipWith<TOther, TResult>(IFlux<TOther> other, Func<T, TOther, TResult> resultSelector);
    ...
    IFlux<IFlux<T>> Window(int count);
    ...
    /// <returns>An <see cref="IFlux{T}"/> scheduled on the specified scheduler.</returns>
    IFlux<T> RunOn(TaskScheduler scheduler);
}
